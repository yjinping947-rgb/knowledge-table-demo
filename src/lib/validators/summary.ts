...
export const summaryRequestSchema = z.object({
  ...
  // 保留本分支：sessionId 的 HMAC 签名（对抗性测试 D1 修复）
...
export const discussionMapSchema = z.object({
  ...
  ripples: z.array(z.object({
    ...
    // 上界放宽到 8 是为了容忍模型偶发多写一两字而不至于整张卡降级；
    // 下界放宽到 2 同理（type/sourceSeat 的枚举保持严格，见 DiscussionMap 类型）。
    label: z.string().trim().min(2).max(8),
